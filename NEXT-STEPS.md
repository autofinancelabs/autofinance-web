...
- [x] **Acceso descubrible a cotizaciones** — RESUELTO: la sidebar/dashboard "Cotizaciones" abren un **listado global** (todas las del asesor) con filtro por cliente/estado (`GET /credit-simulations` sin `clientId`); "Nueva cotización" es un botón dentro. El historial por-cliente sigue en `by-client/:clientId` y el botón por fila en Clientes.
